...
function init_list(files) {
	...
	for(var i in files) {
		if(files[i] != "") {
			...
			html += '</td><td>';
			html += '[<a href="../Am-241/' + field[0] + '">DL</a>] ';
			html += field[1];
			html += '</td></tr>';
		}
	}
...
function sp_info_gps(file_comment) {
	...
	if(file_comment.match(/\[gps:([0-9\.]+),([0-9\.]+)\]/)) {
		var gpslink = '<a href="http://maps.google.co.jp/maps?q='
			+ RegExp.$1 + ',' + RegExp.$2
			+ '&z=18&t=h'
			+ '" target="_blank">在 GoogleMap 上显示位置</a><br>'
			+ '(LATT:' + RegExp.$1
			+ ', LONG:' + RegExp.$2
			+ ')';
		sp_info.push(["位置信息", gpslink]);
	}
	return sp_info;
}
...
function draw_graph_spe_spectrum(file, file_comment, data) {
	...
	if(gps_long && gps_latt) {
		var gpslink = '<a href="http://maps.google.co.jp/maps?q='
			...
			+ ', LONG:' + gps_long
			+ ')';
		spectrum['sp_info'].push(["位置信息", gpslink]);
	}
	return spectrum;
}
...
function draw_graph_emf211_spectrum(file, file_comment, data) {
	...
	if(file_comment.match(/^ *\[#([^\]]+)\]/)) {
		...
		additional_sp_info_html += '<div><img src="../Am-241/' + filename + '.' + ext + '"><br><br></div>';
	}
...
function print_sp_info(spectrum) {
	...
	html += '<p>URL: ';
	html += '<a href="../Am-241/' + url + '" target="_blank">' + url + '</a></p>';
